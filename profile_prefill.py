...
os.environ["VLLM_DISABLE_BARRIERS"]="1"
import torch
import tqdm
import time
...
PROMPT_LEN_DEFAULT = 1024
VOCAB_SIZE_DEFAULT=128280
...
def run_profile(
    ...
    tensorboard_output: Optional[str],
    vocab_size: Optional[int]
):
    batch_size = context.batch_size
    prompt_len = context.prompt_len
    model = context.engine_args.model.split("/")[-1]
    enable_lora = context.engine_args.enable_lora
    tensorboard_output = os.path.join(tensorboard_output, model)
    os.environ["VLLM_TORCH_PROFILER_DIR"] = os.path.join(tensorboard_output, f"{batch_size}_{prompt_len}", "prefill")
    ...
    def add_requests():
        ...
        for i in range(batch_size):
            ...
            prompt_token_ids = torch.randint(
                llm.llm_engine.model_config.get_vocab_size() if not vocab_size else vocab_size, size=(prompt_len,)
            ).tolist()
            ...
            if enable_lora:
                llm.llm_engine.add_request(
                    ...
                    lora_request=LoRARequest("sql_adapter", 1, "/home/ubuntu/models/loras/tat")
                )
            else:
                llm.llm_engine.add_request(
                    request_id=f"seq{i}",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Export the results as a tensorboard file. This should be the folder path",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--vocab-size",
        type=int,
        default=VOCAB_SIZE_DEFAULT,
        help=f"Vocabulary size, sometime get vocab size from tokenizer cause CUDA index error "
        f"default={VOCAB_SIZE_DEFAULT}",
    )
    # parser.add_argument(
    ...
    run_profile(
        ...
        tensorboard_output=args.tensorboard,
