...
inline bool launch_bgmv_expand_sliced_kernel(
    ...
    T **w_ptr,                       // [num_slices] pointers to weight tensors (device)
    const int64_t *indices,          // [num_tokens] per-token lora indices (device)
    const int32_t *d_out_per_slice,  // [num_slices] output dim per slice (HOST memory)
    const int64_t *slice_start_loc,  // [num_slices] column offset per slice (HOST memory)
  ...
  if (uniform_d_out) {
    ...
    // Pass slice_start_loc as host pointer - kernel will use it directly
...
void dispatch_bgmv_shrink_sliced(torch::Tensor y, torch::Tensor x,
  ...
  TORCH_CHECK(ok, "BGMV shrink sliced kernel failed. d_in=", d_in, " d_out=", d_out,
              ". Supported (d_in, d_out) pairs for shrink: (128,8)-(28672,8), (128,16)-(28672,16), (128,32)-(28672,32), (128,64)-(28672,64)");
}
...
void dispatch_bgmv_expand_sliced(torch::Tensor y, torch::Tensor x,
  ...
  CHECK_DIM(1, w_ptr);  // [num_slices]
  CHECK_DIM(1, indices);  // [num_tokens] - per-token lora indices (BGMV style)
  CHECK_DIM(1, slice_start_loc);  // [num_slices] - pre-allocated GPU buffer
  ...
  CHECK_EQ(static_cast<int64_t>(output_slices.size()), num_slices);
  ...
  // Compute d_out_per_slice from output_slices (on CPU, no GPU allocation needed)
  std::vector<int32_t> d_out_host(num_slices);
  for (int64_t i = 0; i < num_slices; ++i) {
    d_out_host[i] = static_cast<int32_t>(output_slices[i]);
  }
  
  // Use the pre-allocated slice_start_loc buffer directly (already on GPU)
  // The caller is responsible for populating this buffer before cudagraph capture
  switch (x.scalar_type()) {
  case at::ScalarType::Half:
  ...
  TORCH_CHECK(ok, "BGMV expand sliced kernel failed. d_in=", d_in, 
              " total_d_out=", total_d_out,
              " first_d_out=", d_out_host[0],
              " num_slices=", num_slices,
              ". Supported (d_in, d_out) pairs for expand: (8,128)-(8,28672), (16,128)-(16,28672), (32,128)-(32,28672), (64,128)-(64,28672)");
}