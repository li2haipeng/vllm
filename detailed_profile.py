...
from layerwise_profiler import layerwise_profile
# from vllm.profiler import layerwise_profile
from vllm.utils import FlexibleArgumentParser

BATCH_SIZE_DEFAULT = 1
PROMPT_LEN_DEFAULT = 256
VOCAB_SIZE_DEFAULT=128000
...
def run_profile(
    ...
    with layerwise_profile(
        on_trace_ready=(
            torch.profiler.tensorboard_trace_handler(os.path.join(tensorboard_output, "prefill"))
    ...
    for _ in tqdm.tqdm(range(num_steps_to_profile - 1)):
        ...
        #     on_trace_ready=(
        #         torch.profiler.tensorboard_trace_handler(os.path.join(tensorboard_output, "decode"))
        #         if tensorboard_output is not None
        #         else None
        #     ),
        # ) as decode_prof:
    ...
    print("=" * LINE_WIDTH)
    print(
        f"= Prefill Model Table " f"(prompt_len={prompt_len}, batch_size={batch_size})"
    )
    print("=" * LINE_WIDTH)
    print()
