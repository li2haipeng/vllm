...
@dataclass
class CutlassLoRAKernelMeta:
    """
    Metadata for CUTLASS SGMV kernels.
    
    CUTLASS kernels require additional handling:
    - token_indices_sorted needs to be int64 for index_select
    - Additional buffers for cudagraph compatibility
    """
    token_lora_mapping: torch.Tensor
    token_indices_sorted_by_lora_ids: torch.Tensor
    token_indices_sorted_int64: torch.Tensor  # For index_select (requires int64)
    active_lora_ids: torch.Tensor
    num_tokens_per_lora: torch.Tensor
    lora_token_start_loc: torch.Tensor
    no_lora_flag_cpu: torch.Tensor
    ...
    def make(
        ...
        # int64 version for index_select
        token_indices_sorted_int64 = torch.empty(
        ...
        return CutlassLoRAKernelMeta(
            token_lora_mapping=token_lora_mapping,
            token_indices_sorted_by_lora_ids=token_indices_sorted_by_lora_ids,
            token_indices_sorted_int64=token_indices_sorted_int64,
            active_lora_ids=active_lora_ids,
            num_tokens_per_lora=num_tokens_per_lora,
            lora_token_start_loc=lora_token_start_loc,
            no_lora_flag_cpu=no_lora_flag_cpu,
    ...
    def prepare_tensors(self, token_lora_mapping: torch.Tensor) -> None:
        ...
        if no_lora:
            # For CUTLASS, we need valid indices even in no-lora case
        ...
        # Also copy to int64 buffer for index_select
        self.token_indices_sorted_int64[:num_tokens].copy_(
            token_indices_sorted_by_lora_ids, non_blocking=True
        )
